...
export function SettingsClient({ initialPrefs }: { initialPrefs: SettingsPrefs }) {
  const { t } = useI18n();

  const [formData, setFormData] = useState({
    minValueUsd: initialPrefs.minValueUsd || 0,
    allowedCategories: initialPrefs.allowedCategories?.length
      ? initialPrefs.allowedCategories
      : ALL_CATEGORIES,
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto pb-20 animate-in fade-in duration-300">
      <div className="flex items-center justify-between pb-4 border-b border-[var(--border-subtle)]">
        <div>
          <h1 className="text-2xl font-bold font-mono tracking-widest text-[var(--accent-yellow)]">
            {t('settings.title')}
          </h1>
          <p className="text-[var(--text-muted)] font-mono text-xs mt-1">
            Global logic settings for claim engine and UI bounds.
          </p>
        </div>
        <ActionButton
          variant="primary"
        ...
        <TerminalCard title={t('settings.general')} borderColor="var(--border-subtle)">
          <div className="flex flex-col gap-6 font-mono text-sm">
            <div className="flex flex-col gap-2">
              <label className="text-[var(--text-dim)] uppercase tracking-widest text-xs">
                &gt; {t('settings.language')}
              </label>
              <select
          ...
          <div className="flex flex-col gap-6 font-mono text-sm">
            <div className="flex flex-col gap-2 p-3 border border-[var(--border-subtle)] rounded-lg bg-black/20">
              <div className="flex items-center justify-between">
                <label className="text-[var(--text-primary)] uppercase tracking-widest text-xs font-bold">
                ...
                <div
                  ...
                  onClick={() =>
                    setFormData({ ...formData, mode: formData.mode === 'clean' ? 'grey' : 'clean' })
                  }
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${formData.mode === 'grey' ? 'translate-x-6' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              {formData.mode === 'grey' && (
                <div className="mt-2 p-2 bg-[rgba(255,68,68,0.1)] border border-[var(--accent-red)] rounded text-[var(--accent-red)] text-xs flex gap-2">
                  ...
                  <span>
                    GREY mode strips identifying data payloads. Highly dangerous if crawler bounds
            ...
            <div
              className={`flex flex-col gap-4 p-3 border ${formData.autoClaimEnabled ? 'border-[var(--accent-green)] bg-[rgba(0,255,136,0.02)]' : 'border-[var(--border-subtle)] bg-black/20'} rounded-lg transition-colors`}
            >
              <div className="flex items-center justify-between">
                <label
                  className={`uppercase tracking-widest text-xs font-bold ${formData.autoClaimEnabled ? 'text-[var(--accent-green)]' : 'text-[var(--text-primary)]'}`}
                >
                  &gt; {t('settings.autoClaim')}
                </label>
                <div
                  ...
                  onClick={() =>
                    setFormData({ ...formData, autoClaimEnabled: !formData.autoClaimEnabled })
                  }
                >
                  <div
                    className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform ${formData.autoClaimEnabled ? 'translate-x-6' : 'translate-x-0'}`}
                  />
                </div>
              </div>
              ...
              <div
                className={`flex flex-col gap-2 mt-2 transition-opacity ${!formData.autoClaimEnabled ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}
              >
                <div className="flex justify-between items-center text-[10px] text-[var(--text-dim)] uppercase">
                  ...
                  <span className="text-[var(--accent-green)] font-bold text-sm">
                    {formData.autoClaimMinScore} / 100
                  </span>
                </div>
                <input
                  type="range"
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, autoClaimMinScore: Number(e.target.value) })
                  }
