...
import { ArrowLeft, Loader2, CheckCircle2, XCircle, AlertTriangle, ShieldCheck, RotateCcw } from "lucide-react";
...
export function FreebieDetailClient({ freebie, breakdown, explanation }: { freebie: any, breakdown: Record<string, number>, explanation: string[] }) {
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({ type: 'ERROR', title: 'OPERATION FAILED', message: error.error || 'Unknown error' });
      }
    } catch(err: any) {
      toast({ type: 'ERROR', title: 'NETWORK ERROR', message: err.message });
    } finally {
      setClaiming(false);
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-5xl mx-auto pb-20 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between border-b border-[var(--border-subtle)] pb-4">
        <ActionButton variant="secondary" onClick={() => router.push('/freebies')} className="text-xs px-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {freebie.tier && (
            <Badge variant={freebie.tier === 'A' ? 'tier-a' : freebie.tier === 'B' ? 'tier-b' : 'tier-c'}>
              TIER {freebie.tier}
            </Badge>
          )}
        ...
        <div className="lg:col-span-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap items-center gap-4 border-y border-[var(--border-subtle)] py-3 px-3 -mx-3 bg-black/20">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-xs text-[var(--text-dim)] uppercase">Category:</span>
                  <Badge variant="category">{freebie.category || t('freebies.unknown')}</Badge>
                </div>
                {freebie.valueUsd && (
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs text-[var(--text-dim)] uppercase">Value:</span>
                    <span className="font-mono text-sm text-[var(--accent-green)] font-bold">${freebie.valueUsd}</span>
                  </div>
                )}
                {freebie.expiry && (
                  <div className="flex items-center gap-2">
                    <span className="font-mono text-xs text-[var(--text-dim)] uppercase">Expiry:</span>
                    <span className="font-mono text-sm text-[var(--accent-yellow)]">{new Date(freebie.expiry).toISOString().split('T')[0]}</span>
                  </div>
                )}
              </div>
          ...
          <TerminalCard title="EXECUTION STEPS" borderColor="var(--border-subtle)">
             <div className="flex flex-col gap-3 font-mono text-sm">
                {freebie.stepsJson ? (
                  Array.isArray(JSON.parse(freebie.stepsJson)) ? JSON.parse(freebie.stepsJson).map((step: string, i: number) => (
                  ...
                  <div className="text-[var(--text-dim)] italic">&gt; NO EXECUTION STEPS DETECTED_</div>
                )}
             </div>
          </TerminalCard>
        </div>
          ...
          <TerminalCard title="AI ENGINE ANALYSIS" borderColor="var(--accent-blue)">
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-3">
                <h3 className="font-mono text-xs text-[var(--text-dim)] uppercase tracking-widest border-b border-[var(--border-subtle)] pb-1">{t('detail.scoreBreakdown')}</h3>
                <div className="grid grid-cols-2 gap-2 font-mono text-xs">
                  {Object.entries(breakdown).map(([key, val]) => (
                    <div key={key} className="flex flex-col bg-[var(--bg-base)] border border-[var(--border-subtle)] p-2 rounded">
                      <span className="text-[var(--text-muted)] text-[9px] mb-1 truncate">{formatKey(key)}</span>
                      <span className={`font-bold ${val > 0 ? 'text-[var(--accent-green)]' : val < 0 ? 'text-[var(--accent-red)]' : 'text-[var(--text-dim)]'}`}>
                        {val > 0 ? `+${val}` : val}
                      </span>
                    </div>
                  ))}
              ...
              <div className="flex flex-col gap-2 font-mono text-xs">
                <h3 className="text-[var(--text-dim)] uppercase tracking-widest border-b border-[var(--border-subtle)] pb-1 mb-1">Evaluation Matrix</h3>
                {explanation.map((exp, i) => {
                  const isPositive = exp.includes('(+');
                  const isWarning = exp.includes('(-');
                  return (
                    <div key={i} className={`flex items-start gap-2 ${isPositive ? 'text-[var(--accent-green)]' : isWarning ? 'text-[var(--accent-red)]' : 'text-[var(--text-muted)]'}`}>
                      <span className="mt-0.5 shrink-0">
                        {isPositive ? <CheckCircle2 className="w-3.5 h-3.5" /> : isWarning ? <XCircle className="w-3.5 h-3.5" /> : <span className="opacity-50">-</span>}
                      </span>
          ...
          <TerminalCard title="COMMAND INTERFACE" borderColor="var(--accent-yellow)">
            <div className="flex flex-col gap-3">
              {freebie.status !== 'claimed' && (
                ...
                >
                  {claiming ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <span className="mr-2 inline-block">✓</span>}
                  MARK AS CLAIMED
                </ActionButton>
              )}
              ...
              {freebie.status !== 'ignored' && freebie.status !== 'claimed' && (
                <ActionButton 
                  ...
                  disabled={claiming || ignoring || rescoring}
                >
                  {ignoring ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <XCircle className="w-4 h-4 mr-2" />}
                  IGNORE DEAL
                </ActionButton>
              )}
              ...
              <ActionButton 
                ...
                disabled={claiming || ignoring || rescoring}
              >
                {rescoring ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <RotateCcw className="w-4 h-4 mr-2" />}
                ↺ FORCE RESCORE
              </ActionButton>
            </div>
          </TerminalCard>
