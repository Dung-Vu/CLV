...
function FreebiesContent() {
  ...
  const [deals, setDeals] = useState<FreebieListItem[]>([]);
  ...
  const handleClaim = (deal: FreebieListItem) => {
    toast({
      type: 'SUCCESS',
      title: 'CLAIM INITIATED',
      message: `Executing claim flow for ${deal.title.substring(0, 25)}...`,
    });
  };
  ...
  useKeyboardShortcuts({
    ...
    onX: () => {
      if (focusedIndex >= 0 && focusedIndex < deals.length) {
        toast({
          type: 'INFO',
          message: `Deal [${deals[focusedIndex].title.substring(0, 15)}...] ignored.`,
        });
      }
    },
    onEnter: () => {
  ...
  const handleTouchMove = (e: React.TouchEvent, deal: FreebieListItem) => {
  ...
  const handleTouchEnd = (_e: React.TouchEvent, deal: FreebieListItem) => {
    if (!touchStartRef.current || touchStartRef.current.id !== deal.id) return;
  ...
  return (
    <div className="flex flex-col gap-6 w-full animate-in fade-in duration-300 pb-20">
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-3 w-full scrollbar-thin scrollbar-thumb-[var(--border-subtle)]">
          ...
          <div className="flex items-center gap-2 shrink-0 border-l border-[var(--border-subtle)] pl-4">
            ...
            <div className="relative">
              ...
              <ChevronDown
                size={14}
                className="absolute right-2 top-1/2 -translate-y-1/2 text-[var(--accent-green)] pointer-events-none"
              />
            </div>
          </div>
      ...
      {/* CONTENT AREA */}
      <div
        className={`relative min-h-[50vh] ${loading ? 'opacity-50 pointer-events-none grayscale-[50%]' : 'opacity-100'} transition-all duration-300`}
      >
        {loading && deals.length === 0 && (
          <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none pb-20">
            ...
            <div className="font-mono text-[var(--text-dim)] text-sm terminal-cursor mb-4">
              &gt; {t('freebies.fetching')}
            </div>
          </div>
        )}
        ...
        {deals.length === 0 && !loading ? (
          ...
          <>
            <div
              className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 ${viewMode === 'table' ? 'hidden md:hidden max-md:grid' : ''}`}
            >
              {deals.map((deal, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`absolute inset-0 flex items-center font-mono text-sm tracking-widest font-bold ${swipeState.id === deal.id && swipeState.offset > 0 ? 'bg-[var(--accent-green)] text-black justify-start pl-6' : 'bg-[var(--accent-red)] text-white justify-end pr-6'} transition-opacity duration-200 ${swipeState.id === deal.id && swipeState.offset !== 0 ? 'opacity-100 z-0' : 'opacity-0 -z-10'}`}
                    >
                      {swipeState.id === deal.id && swipeState.offset > 0 ? (
                        <span className="flex items-center gap-2 animate-pulse">
                          <Check className="w-5 h-5" /> CLAIM TIER A
                        </span>
                      ) : (
                        <span className="flex items-center gap-2 animate-pulse">
                          IGNORE DELETED <XIcon className="w-5 h-5" />
                        </span>
                      )}
                    </div>
                    ...
                    <div
                      style={{
                        transform:
                          swipeState.id === deal.id
                            ? `translateX(${swipeState.offset}px)`
                            : 'translateX(0)',
                        transition:
                          swipeState.id === deal.id && !swipeState.isDragging
                    ...
                    >
                      <TerminalCard
                        ...
                        className={`h-full group transition-all duration-200 ${
                          isFocused
                            ? 'scale-[1.03] shadow-[0_0_20px_rgba(0,255,136,0.15)] z-10 !border-[var(--accent-green)] border-l-[3px]'
                            : ''
                        }`}
                      >
                        {isFocused && (
                        ...
                        {isFocused && (
                          <div className="absolute top-2 right-3 text-[var(--accent-green)] text-[10px] font-mono animate-[blink_1s_step-end_infinite] font-bold pointer-events-none z-30">
                            &lt;FOCUSED&gt;
                          </div>
                        )}
                        <div className="flex flex-col h-full gap-3">
                          <div className="flex items-start justify-between gap-2">
                            <h3
                              className="font-bold text-[var(--text-primary)] leading-tight flex-1 group-hover:text-white transition-colors cursor-pointer"
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            ...
                            {deal.tier ? (
                              <Badge variant={getTierBadge(deal.tier) as BadgeVariant}>
                                TIER {deal.tier}
                              </Badge>
                            ) : (
                          ...
                          <div className="flex items-center flex-wrap gap-x-4 gap-y-1 font-mono text-[10px] text-[var(--text-dim)] border-t border-[var(--border-subtle)] pt-3 mt-1">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              {deal.expiry
                                ? new Date(deal.expiry).toISOString().split('T')[0]
                                : 'N/A'}
                            </span>
                            <span
                              className={`flex items-center gap-1 ${deal.riskLevel?.toLowerCase() === 'low' ? 'text-[var(--accent-blue)]' : deal.riskLevel?.toLowerCase() === 'high' ? 'text-[var(--accent-red)]' : 'text-[var(--accent-yellow)]'}`}
                            >
                              <span className="text-[var(--text-muted)]">RSK:</span>
                              {deal.riskLevel ? deal.riskLevel.toUpperCase() : 'UNK'}
                            </span>
                          </div>
            ...
            {/* TABLE VIEW */}
            <div
              className={`bg-[var(--bg-surface)] border border-[var(--border-subtle)] rounded-lg overflow-x-auto hidden ${viewMode === 'table' ? 'md:block' : ''}`}
            >
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-[var(--border-subtle)] cursor-pointer transition-colors ${
                          isFocused
                            ? 'bg-[rgba(0,255,136,0.05)] text-[var(--accent-green)] border-l-[3px] border-l-[var(--accent-green)]'
                            : 'hover:bg-[rgba(255,255,255,0.02)] text-[var(--text-primary)] border-l-[3px] border-l-transparent'
                        }`}
                        onClick={() => router.push(`/freebies/${deal.id}`)}
                      >
                        <td className="px-4 py-3 relative">
                          {isFocused && (
                            <span className="absolute left-1 top-1/2 -translate-y-1/2 text-[var(--accent-green)] animate-[blink_1s_step-end_infinite] font-bold">
                              &gt;
                            </span>
                          )}
                          <ScoreBadge score={deal.score || 0} />
                        ...
                        <td className="px-4 py-3 truncate max-w-[300px] whitespace-normal line-clamp-1">
                          {deal.title}
                        </td>
                        <td className="px-4 py-3">
                          <Badge variant="category">{deal.category || 'UNKNOWN'}</Badge>
                        </td>
                        <td className="px-4 py-3 font-bold text-[var(--accent-green)]">
                          ${deal.valueUsd || '0'}
                        </td>
                        <td className="px-4 py-3">
                          <Badge
    ...
    <Suspense
      fallback={
        <div className="flex flex-col items-center justify-center py-40 gap-4">
          ...
          <div className="font-mono text-[var(--text-dim)] text-sm terminal-cursor">
            &gt; INITIALIZING FREEBIES UI...
          </div>
        </div>
      }
    >
