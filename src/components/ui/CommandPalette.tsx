...
import { Search, TerminalSquare, Activity, Target, Zap, Settings, X, Filter } from "lucide-react";
import { toast } from "@/hooks/useToast";

const COMMANDS = [
  { id: "run-ingestion", label: "Run System Ingestion", sub: "Fetch deals from all active sources", icon: Zap, section: "ACTIONS" },
  { id: "run-analyzer", label: "Run AI Analyzer", sub: "Score and classify raw deals", icon: Activity, section: "ACTIONS" },
  { id: "filter-tiera", label: "Filter Tier A Deals", sub: "Show highly recommended freebies", icon: Filter, section: "ACTIONS", href: "/freebies?tier=A" },
  { id: "go-dashboard", label: "Open Command Center", sub: "go to /", icon: TerminalSquare, section: "NAVIGATION", href: "/" },
  { id: "go-freebies", label: "Open Freebies", sub: "go to /freebies", icon: Target, section: "NAVIGATION", href: "/freebies" },
  { id: "go-logs", label: "Open Logs", sub: "go to /logs", icon: TerminalSquare, section: "NAVIGATION", href: "/logs" },
  { id: "go-config", label: "Open Config", sub: "go to /settings", icon: Settings, section: "NAVIGATION", href: "/settings" },
];
...
export function CommandPalette() {
  ...
  const executeCommand = (cmd: typeof COMMANDS[0]) => {
    ...
    if (cmd.href) {
      ...
      toast({ type: 'INFO', title: 'NAVIGATING', message: `Sourced to ${cmd.label.toUpperCase()}` });
          ...
          {results.length > 0 ? (
             results.map((cmd, idx) => {
               ...
               return (
                 ...
                 >
                   <cmd.icon className={`w-4 h-4 shrink-0 ${active ? "text-[var(--accent-blue)]" : "text-[var(--text-dim)]"}`} />
                   <div className="flex flex-col flex-1 min-w-0">
                     <span className={active ? "text-[var(--text-primary)] font-bold tracking-wide" : "text-[var(--text-primary)]"}>
                       {cmd.label}
                     </span>
                     <span className={`text-[10px] truncate ${active ? "text-[var(--accent-blue)] opacity-80" : "text-[var(--text-dim)]"}`}>
                       {cmd.sub}
                     </span>
                   </div>
                   {active && <span className="text-[10px] font-bold px-2 py-1 bg-black/20 rounded border border-[rgba(14,165,233,0.3)] shrink-0">↵ ENTER</span>}
                 </div>
               )
             })
          ) : (
